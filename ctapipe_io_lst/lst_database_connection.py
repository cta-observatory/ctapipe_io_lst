...
# converts from<Plug>PeepOpenostgres to<Plug>PeepOpenython
...
class LSTDatabaseConnection:
    ...
    def load_calibration_data(self, run):
        ...
        calib = {field_name: res_calib[i] for i, field_name in enumerate(waveform_fields)}
        ...
        flatfield = {field_name: res_flatfield[i] for i, field_name in enumerate(flatfield_fields)}
        ...
        pedestal = {field_name: res_pedestal[i] for i, field_name in enumerate(pedestal_fields)}
        ...
        pixel = {field_name: res_pixel[i] for i, field_name in enumerate(pixel_status_fields)}
    ...
    def load_drs4_pedestal_data(self, run):
        self.cursor.execute("SELECT baseline_mean FROM drs4_baseline_pro where run=%s", (run,))
        return self.cursor.fetchone()[0]

    def load_drs4_spike_height_data(self, run):
        self.cursor.execute("SELECT spike_height FROM drs4_baseline_pro where run=%s", (run,))
        return self.cursor.fetchone()[0]

    def load_drs4_time_calibration_data(self, run):
        self.cursor.execute("SELECT fan, fbn FROM \"drs4_time_sampling_from_FF_pro\" where run=%s", (run,))

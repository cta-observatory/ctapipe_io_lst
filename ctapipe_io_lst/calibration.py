...
class LSTR0Corrections(TelescopeComponent):
    ...
    def spike_corr(self, event, tel_id):
        """
        Subtract corresponding spike ADC counts
        ...
        run_id = event.lst.tel[tel_id].svc.configuration_id        
        if run_id > LAST_RUN_WITH_OLD_FIRMWARE:
            subtract_spikes = subtract_spikes_jit
        else:
            subtract_spikes = subtract_spikes_jit_data_from_20181010_to_20191104

        # corresponding spike adc counts are hard-coded for now. Those values will be loaded from a pedestal file.
        spike_adc = np.array([47, 54, 8])
            ...
            if last_lc % 2 == 0 and last_lc<= LAST_IN_FIRST_HALF:
                ...
                for k in range(3):
                    ...
                    if (spike_pos - current_fc) % N_CAPACITORS_CHANNEL < N_SAMPLES:
                        ...
                        waveform[gain, pixel,(spike_pos - current_fc) % N_CAPACITORS_CHANNEL] -= spike_adc[k]
...
@njit(cache=True)
def subtract_spikes_jit_data_from_20181010_to_20191104(
    waveform,
    first_capacitor,
    first_capacitor_old,
    spike_flag,
    ...
    for gain in range(N_GAINS):
        for pixel in range(N_PIXELS):
            ...
            if last_lc % 2 == 0 and last_lc<= LAST_IN_FIRST_HALF:
                ...
                for k in range(3):
                    spike_pos = (last_lc -1 + k) % N_CAPACITORS_CHANNEL
